...
void set_bnd ( int N, int b, float * x )
{
	...
	for ( i=1 ; i<=N ; i++ ) {
		...
		x[IX(N+1,i)] = 0; // b==1 ? -x[IX(N,i)] : x[IX(N,i)];
		x[IX(i,0  )] = 0; // b==2 ? -x[IX(i,1)] : x[IX(i,1)];
		x[IX(i,N+1)] = 0; // b==2 ? -x[IX(i,N)] : x[IX(i,N)];
	}
	x[IX(0  ,0  )] = 0; // 0.5f*(x[IX(1,0  )]+x[IX(0  ,1)]);
	x[IX(0  ,N+1)] = 0; // 0.5f*(x[IX(1,N+1)]+x[IX(0  ,N)]);
	x[IX(N+1,0  )] = 0; // 0.5f*(x[IX(N,0  )]+x[IX(N+1,1)]);
	x[IX(N+1,N+1)] = 0; // 0.5f*(x[IX(N,N+1)]+x[IX(N+1,N)]);
}
...
void dump_density ( int step_count, int N, float * d )
{
        int i, j;
        char str[256];
